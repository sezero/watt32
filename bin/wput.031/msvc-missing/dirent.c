...
DIR *opendirx(char *name, char *pattern)
{
    ...
    do
    {
      ...
      else dirp->dd_contents = dirp->dd_cp = dp;
...
struct dirent *readdir(DIR *dirp)
{
    ...
    g_dp.d_name = dirp->dd_cp->_d_entry;

    g_dp.d_namlen = g_dp.d_reclen = (int) strlen(g_dp.d_name);

    g_dp.d_ino = (short) (dirp->dd_loc + 1);    /* fake the inode */
...
static char *getdirent(char *dir)
{
   ...
   {
      FindHandle = FindFirstFile (dir, &FileFindData);
