...
#if defined(_WIN32) && !defined(USE_WATT32) /* Raw sockets in Windows stinks */
  ttl = index + 1;
...
/*   We got a return on something we sent out.  Record the address and
     time.  */
void net_process_ping(int seq, uint32_t addr, struct timeval now) {
  int index;
  int totusec;
...
int net_preopen() {
  ...
  {
    fprintf (stderr, "Failed to initialise Winsock. Error %lu\n", GetLastError());
    return -1;
  }
